        ...
        try {
            ...
            if (records && records.length > 0) {
                ...
                const isRealData = firstRecord && 
                    ...
                    firstRecord['计划ID'] !== 'LOCAL_' + Math.random();
            ...
            console.log('⚠️ API返回数据为空或非真实数据');
            throw new Error('数据库连接成功但无数据');
    ...
    async getAllDataWithProgress(progressCallback, options = {}) {
        ...
        try {
            ...
            const params = {
                ...
                satellite_name: options.satellite_name,
    ...
    async hasData() {
        ...
        try {
            ...
            if (records && records.length > 0) {
                ...
                const isRealData = firstRecord && 
                    !firstRecord.plan_id?.startsWith('SAMPLE_') &&
                    firstRecord.plan_id !== 'LOCAL_' + Math.random();
